...
public class GroundedCheck
{
    ...
    public GroundedCheck(GameObject s, Rigidbody2D rigid, float distance)
    {
        ...
        size = new Vector3(coll.bounds.size.x, 2*coll.bounds.size.y, coll.bounds.size.z);
        rb = rigid;
        d = distance;
    }
    ...
    {
        if (Physics2D.BoxCast(coll.bounds.center, size, 0.0f, Vector2.down, d, ~(1 << source.layer)) && rb.velocity.y <= 0.01) return true;
